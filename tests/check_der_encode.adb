...
package body Check_DER_Encode is
   ...
   procedure Test_Put_Length(T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Test_Cases : array(1 .. 10) of Test_Case :=
        ( 1 => (Input => 0,
   ...
   begin
      for I in Test_Cases'Range loop
         declare
            Result : Hermes.Octet_Array := Put_Length_Value(Test_Cases(I).Input);
         begin
            Assert
              (Result'Length = Test_Cases(I).Expected.Size,
               "Test case #" & Integer'Image(I) & " length failed");
            Assert
              (Result = Test_Cases(I).Expected.Value,
   ...
   procedure Test_Put_Boolean(T : in out AUnit.Test_Cases.Test_Case'Class) is
      Boolean_False : Hermes.Octet_Array := (16#01#, 16#01#, 16#00#);
      Boolean_True  : Hermes.Octet_Array := (16#01#, 16#01#, 16#FF#);
   begin
   ...
   begin
      for I in Test_Cases'Range loop
         declare
            Result : Hermes.Octet_Array := Put_Integer_Value(Test_Cases(I).Input);
         begin
            Assert
              (Result'Length = Test_Cases(I).Expected.Size,
               "Test case #" & Integer'Image(I) & " length failed");
